...
class Bilinear(Layer):
    ...
    def __init__(self, incoming, downsample_factor=1,
                 control_points=4, precompute_grid='auto', **kwargs):
        super(Bilinear, self).__init__(incoming, **kwargs)

        self.downsample_factor = as_tuple(1/np.asarray(downsample_factor, dtype='float32'), 2)
        self.control_points = control_points
        ...
        # Create source points and L matrix
        self.right_mat, self.L_inv, self.source_points, self.out_height, \
            self.out_width = _initialize_bilinear(
                control_points, input_shp, self.downsample_factor,
                precompute_grid)
    ...
    def get_output_for(self, inputs, **kwargs):
        ...
        # Get input and destination control points
        input  = inputs
        dest_offsets = T.zeros((input.shape[0], 2 * self.control_points))
        return _transform_bilinear(
                dest_offsets, input, self.right_mat, self.L_inv,
                self.source_points, self.out_height, self.out_width,
                self.precompute_grid, self.downsample_factor)

def _bilinear(im, x, y, out_height, out_width):
    # *_f are floats
    num_batch, height, width, channels = im.shape
    height_f = T.cast(height, theano.config.floatX)
    width_f = T.cast(width, theano.config.floatX)
...
def _initialize_bilinear(num_control_points, input_shape, downsample_factor,
                    precompute_grid):
    """
